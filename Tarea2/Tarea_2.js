...
// Primitives use arrays of data, called buffers, which define the positions of the vertices to be drawn.
    ...
    //Creamos funcion para la primer Figura(Cuadrado)
...
function createSquare(gl) 
{
    ...
    // Creamos funcion para la segunda Figura(Triangulo)
function createTriangle(gl)
{
    ...
    //Creamos funcion para la tercer Figura(Rombo)
function createRombo(gl)
{
    ...
    //Creamos funcion para crear la cuarta figura(Pacman)
function createPacman(gl)
{
    let vertexBuffer;
    vertexBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, vertexBuffer);
    let verts=createCirculo(0,0,0,0.5,26);

    
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(verts), gl.STATIC_DRAW);
    
    //usamos la primitiva Triangle_Fan para poder dibujar y unir los vertices
    let pacman = {buffer:vertexBuffer, vertSize:3, nVerts:26, primtype:gl.TRIANGLE_FAN};
    return pacman;
} 

    //Creamos la funcion para crear un circulo y poder crear el Pacman
function createCirculo(X,Y,Z,radioCirculo,vertices){
    
 
    let medidaCirculo=2*Math.PI;
    ...
    for(var i =2;i<vertices;i++){
        VerticesX.push(X+(radioCirculo*Math.cos(i*medidaCirculo/28)));
        VerticesY.push(Y+(radioCirculo*Math.sin(i*medidaCirculo/28)));
        VerticesZ.push(Z);
    }
...
let Circulo=[]
    for (var i=0;i<vertices;i++){
        Circulo.push(VerticesX[i]);
        Circulo.push(VerticesY[i]);
        Circulo.push(VerticesZ[i]);
    }
